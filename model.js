...
//
// Al arrancar la aplicación la variable contiene estas 4 preguntas,
...
//------------------ Autoguardado de preguntas y respuestas -----------------
/**
 * Cargar las preguntas guardadas en el fichero.
 *
 ...
 * existe, y se producirá el error ENOENT. een este caso se salva el 
 * contenido inicial almacenado en quizzes.
 * Si se porudce  otro tipo de eeror, se lanza una excepción que abortará
 * la ejecucción del programa.
 */
const load = () => {
	fs.readFile(DB_FILENAME, (err, data) => {
		if(err){
			// La primera vez no existe el fichero (ENOENT)
			if (err.code === "ENOENT"){
 ...
 * 
 * Guardar en formato JSON el valor de quizzes en el fichero DB_FILENAME.
 * Si se produce algún tipo de error, se lanza una excepcion que abortará
 ...
 * 
 * @returns {numbers} número total de preguntas existentes.
 */
...
// Cargar los quizzes almacenados en el fichero.
load();
